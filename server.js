...
async function getLinks() {
  try { return await kv.get('links') || {}; } catch (e) { return {}; }
}
async function setLinks(links) {
  try { await kv.set('links', links); } catch (e) {}
}
async function getStats(code) {
  try { return await kv.get(`stats:${code}`) || []; } catch (e) { return []; }
}
...
async function deleteLink(code) {
  ...
  try { await kv.del(`stats:${code}`); } catch (e) {}
}
...
// ==================== VERCEL EXPORT ====================
module.exports = app;    const stats = await getStats(code);
    stats.push(entry);
    if (stats.length > 500) stats.shift();
    await kv.set(`stats:${code}`, stats);
  } catch (e) {}
